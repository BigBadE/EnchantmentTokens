...
public class ArmorEquipListener extends BasicEnchantListener implements Listener {
    ...
    public void onArmorEquip(ArmorEquipEvent event) {
        ...
        if(item != null) {
            CustomEnchantEvent enchantmentEvent = new CustomEnchantEvent(ListenerType.UNEQUIP, item).setUser(event.getPlayer());
            callListeners(enchantmentEvent, oldArmorListeners);
        }
        item = event.getNewArmorPiece();
        if(item != null) {
            CustomEnchantEvent enchantmentEvent = new CustomEnchantEvent(ListenerType.EQUIP, item).setUser(event.getPlayer());

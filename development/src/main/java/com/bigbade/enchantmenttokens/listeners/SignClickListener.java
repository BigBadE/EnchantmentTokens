...
public class SignClickListener implements Listener {
    ...
    public void onSignInteract(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getHand() == EquipmentSlot.HAND
                && event.getClickedBlock() != null && event.getClickedBlock().getState() instanceof Sign) {
            Sign sign = (Sign) event.getClickedBlock().getState();
            EnchantmentPlayer enchantmentPlayer = playerHandler.getPlayer(event.getPlayer());
            if (ChatColor.stripColor(sign.getLine(0)).equals("["
                    + LocaleMessages.ENCHANTMENT.translate(enchantmentPlayer.getLanguage()) + "]")) {
                ItemStack itemStack = event.getItem();
                EnchantUtils.getInstance().addEnchantment(enchantmentPlayer, handler, itemStack, sign.getLine(1));
                event.getPlayer().sendSignChange(event.getClickedBlock().getLocation(),
                        new String[]{sign.getLine(0), sign.getLine(1), "", ""});
            }
        }
    }
}

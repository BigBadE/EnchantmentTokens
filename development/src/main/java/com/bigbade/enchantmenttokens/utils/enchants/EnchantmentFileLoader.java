...
public class EnchantmentFileLoader {
    ...
    private static Class<?> loadClass(JarEntry jar, ClassLoader loader) throws ClassNotFoundException {
        ...
        className = RegexPatterns.DIVIDER_PATTERN.matcher(className).replaceAll(".");
    ...
    private void finishLoading(long start) {
        LocaleManager.updateLocale(addons);
        EnchantmentTokens.getEnchantLogger().log(Level.INFO, "Loaded enchantments in {0}ms",

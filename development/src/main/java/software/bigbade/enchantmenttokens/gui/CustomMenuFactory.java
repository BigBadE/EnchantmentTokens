...
public class CustomMenuFactory implements EnchantmentMenuFactory {
    ...
    public SubInventory generateGUI(ItemStack itemStack, EnchantmentPlayer player) {
        Inventory inventory = Bukkit.createInventory(null, 54, CustomMenuFactory.getName(itemStack, player.getLanguage()));
    ...
    public EnchantmentGUI genItemInventory(EnchantmentPlayer enchantPlayer, ItemStack item, List<EnchantmentBase> added) {
        Inventory inventory = Bukkit.createInventory(null, 9 * (2 + ((int) Math.ceil(buttons.size() / 7f))), CustomMenuFactory.getName(item, enchantPlayer.getLanguage()));
        ItemMeta meta = item.getItemMeta();
        if (meta == null) {
